...
SELECT
  ...
  -- Rename lei (Legal Entity Identifier) to something readable —
  -- this is the unique ID for the lending institution
  "lei" AS lender_id,
  -- Keep geography fields as-is, just renamed for consistency
  "county_code" AS county_code,
  "state_code" AS state_code,
   -- Cast year to a proper number type (was inferred, but this
    -- makes the type explicit and safe for later date math)
  "action_taken"::NUMBER AS action_taken,

      -- Cast action_taken to a number — this is our key outcome
